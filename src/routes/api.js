...
// ─── Calls (with date range, pagination, voicemail hiding) ──
router.get('/calls', async (req, res) => {
  try {
    ...
    // Date range filtering (from/to take priority over period)
...
router.delete('/calls/:id', async (req, res) => {
  try {
    await q('DELETE FROM calls WHERE id=?', [req.params.id]);
    res.json({ ok: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.get('/analytics', async (req, res) => {
  try {
    ...
    for (const row of catAvgs.rows) {
      ...
      for (const cat of ['discovery','qualification','pitch','objections','close_next_step']) {
        if (cs[cat] != null) repCats[row.rep_name][cat].push(Number(cs[cat]));
      }
    }
    const categoryAvgs = {};
    for (const [rep, cats] of Object.entries(repCats)) {
      ...
      for (const [cat, vals] of Object.entries(cats)) {
        categoryAvgs[rep][cat] = vals.length ? +(vals.reduce((a,b)=>a+b,0)/vals.length).toFixed(1) : null;
      }
    }
...
router.get('/analytics/trends', async (req, res) => {
  try {
    ...
    if (period === 'weekly') {
      ...
      dateExpr = "date(received_at)";
      groupBy = "date(received_at)";
    }

    const trends = await q(`SELECT ${dateExpr} as period_date,
      ...
      FROM calls WHERE received_at >= date('now','-${Number(days)} days') ${rf}
      GROUP BY ${groupBy} ORDER BY period_date ASC`, p);

    // Per-rep trends
    const repTrends = await q(`SELECT ${dateExpr} as period_date, rep_name,
      COUNT(*) as calls, ROUND(AVG(overall_score_adj),1) as avg_score
      FROM calls WHERE status='SCORED' AND received_at >= date('now','-${Number(days)} days') ${rf}
      GROUP BY ${groupBy}, rep_name ORDER BY period_date ASC`, p);
